...
object CurriesPAF extends App {
  ...
  def curriedFormater(s: String)(number: Double) = s.format(number)
  val list = List(Math.PI, Math.E, 1, 9.8, 1.3e-12)
  ...
  println(list.map(simpleFormat))
  println(list.map(seriousFormat))
  println(list.map(preciseFormat))
